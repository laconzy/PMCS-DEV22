...
class Bundle extends CI_Controller
{
	...
	public function fg_bundle()
	{
		...
		$this->load->view('cutting/fg_bundle_creation', $data);
	}

	public function get_laysheet_details($laysheet_no)
	{

		$laysheet = $this->bundle_model->get_laysheet_details($laysheet_no);
	...
	public function get_order_details($laysheet_no)
	{
		...
		$laysheet = $this->bundle_model->get_order_details($laysheet_no);

		$cut_details = $this->bundle_model->get_order_item_details($laysheet_no);

		//$cut_plan = $this->bundle_model->get_cut_plan($laysheet['cut_plan_id']);
		$cut_plan = $this->bundle_model->get_cut_plan(30);
	...
	public function save()
	{
		...
		$layshhe_details = $this->bundle_model->get_laysheet_details($laysheet_no);

		$cut_plan = $this->bundle_model->get_cut_plan($layshhe_details['cut_plan_id']);

		$this->generate_bundles($laysheet_no, $plies, $layshhe_details['order_id'], $cut_plan['style'], $cut_plan['color'], $cut_plan['item_id'], $cut_no,$site,$shift);

		$bundle_chart = $this->bundle_model->get_bundle_chart($laysheet_no);
	...
	{
		//$laysheet_no = $this->input->post('laysheet_no');
		$bundle_chart = $this->bundle_model->get_saved_details($laysheet_no);
	...
	public function save_fg_bundle()
	{
		...
		$id = $this->bundle_model->save_fg_barcode($ord_id);
		$result = $this->bundle_model->save_fg_barcode_detail($id);
	...
	public function destroy($laysheet_no, $bundle_no)
	{
		...
		$this->bundle_model->destroy($laysheet_no, $bundle_no);
	...
	public function print_data($bundle_no, $qty)
	{
		...
		$start = $this->bundle_model->print_data($bundle_no, $qty);
	...
	{
		$count=$this->bundle_model->count_scan_all($laysheet_no);
		...
		$this->bundle_model->destroy_all($laysheet_no);
	...
	public function get_bundle_chart($laysheet)
	{
		...
		$bundle_chart = $this->bundle_model->get_bundle_chart($laysheet);
	...
	public function print_barcode($laysheet_no)
	{
		...
		$this->load->view('cutting/bundle_barcode', $data);
	}

	public function print_fg_barcode($id, $start)
	{

		$barcode = $this->bundle_model->get_barcode_details($id, $start);
//        $laysheet = $this->bundle_model->get_laysheet_details($laysheet_no);
//f
//        $cut_details = $this->bundle_model->get_cut_details_summery($laysheet_no);
//
...
//
	...
	private function generate_bundles($laysheet, $plies, $order_id, $style, $color, $item, $cut_no,$site,$shift=null)
	{
		...
		$sizeList = $this->bundle_model->get_laysheet_size_details($laysheet);

		$bundle_no = $this->bundle_model->get_laysheet_max_bundle_no($laysheet);

		$remaning_size_qty = $this->bundle_model->get_laysheet_remaning_qty($laysheet);
...
//                }
		...
		$this->bundle_model->create($genarated_bundles);
