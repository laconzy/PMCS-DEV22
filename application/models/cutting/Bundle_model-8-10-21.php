...
class Bundle_Model extends CI_Model
{
	...
	public function count_scan($laysheet_no, $bundle_no)
	{

		$sql = "SELECT
			IFNULL(COUNT(production.qty),0) c_qty
			FROM `production`
			
			WHERE
production.laysheet_no = '" . $laysheet_no . "' AND
production.bundle_no = '" . $bundle_no . "'";
	...
	public function count_scan_all($laysheet_no)
	{
		...
		$sql = "SELECT
			...
			FROM `production`			
			WHERE
			production.laysheet_no = " . $laysheet_no;
		$query = $this->db->query($sql);
 ...
 fg_barcode_head.ord_id =$ord_id ";
	...
	public function print_data($bundle_no, $qty)
	{
		...
		for ($index = $start; $index < $end; $index++) {
			$serial=sprintf('%05d', $index);
			$data = [
				'id' => $bundle_no,
				'barcode_no' => 'M' . $bundle_no . $serial,
