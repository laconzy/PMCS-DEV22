...
public function get_production_detail($building)
  {
        ...
        $sql = "SELECT
           ...
           IFNULL(production_dashboard.`11`,0) '11',
            ...
            if($building == "BC"){
              $sql .= "production_dashboard.section IN ('B','C')";
            }
            else {
              $sql .= "production_dashboard.section = '" . $building . "'";
            }
  ...
  public function get_max_hour(){
    $cur_date = date("Y-m-d");
    $sql = "SELECT MAX(production.`hour`) AS hour FROM production WHERE production.scan_date = '".$cur_date."'";
    $query = $this->db->query($sql);
    $result = $query->result_array();
    //echo json_encode($result[0]["hour"]);
    return $result[0]["hour"] == null ? 0 : $result[0]["hour"];
  }
