	...
	$(document).ready(function(){
		...
		let data = localStorage.getItem('pmcs_inv_data');
		data = JSON.parse(data);
		load_labels(data);

	});

	function load_labels(packing_id){
		appAjaxRequest({
			url: BASE_URL + 'index.php/inventory/inventory/get_packing_labels_data',
			type: 'post',
			dataType: 'json',
			data : {
		...
		for(let x = 0 ; x < data.length ; x++){
			str = `<div class="sticker" align="center">
				<table width="100%" border="0" style="margin-top:2px;">
					<tr>
						<td colspan="2" class="status">${data[x].packing_list_no}</td>
					</tr>
					<tr>
						<td class="part-no">Part No</td>
						<td class="part-no">${data[x].part_no}</td>
					</tr>
					<tr>
						<td>Description</td>
					...
					<tr>
						<td class="po-no">PO No/Buyer</td>
						<td class="po-no">${data[x].po_no} / ${data[x].buyer_code}</td>
					</tr>
					<tr>
						<td>Date</td>
						<td>${data[x].ins_date} - Invoice : ${data[x].invoice}</td>
					</tr>
					<tr>
						<td>Container</td>
						<td>${data[x].fng_no}</td>
					...
					<tr>
						<td class="part-no">Style</td>
						<td class="part-no">${data[x].style} - (BIN : ${data[x].bin})</td>
					</tr>
					<tr>
						<td class="part-no">Batch No</td>
						<td class="part-no">${data[x].batch_no}</td>
					</tr>
					<tr>
