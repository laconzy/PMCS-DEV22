...
<head>
	...
	<title>PMCS | Size Wise Order Status Report</title>
...
<div>
	<h4 style="margin-bottom:20px;text-align:center;font-weight:bold">ORDER STATUS REPORT - SIZE WISE</h4>
	...
	$(document).ready(function () {
		...
		$('#btn_search').click(function () {
		  ...
		  animation: 'cubeGrid'
			...
			appAjaxRequest({
				url: BASE_URL + 'index.php/report/order_status_report_size_wise_data',
				type: 'GET',
				dataType: 'json',
				//async: false,
				data: {
  ...
  function ExportToExcel(type, fn, dl) {
		if(DATASET != null && DATASET.length > 0){
			...
			for(let x = 0 ; x < DATASET.length ; x++){
				...
				<td>${format_data(DATASET[x]['PREPARATION'])}</td>
				<td>${supermarket_in}</td>
				<td>${cut_out - supermarket_in}</td>
				<td>${supermarket_out}</td>
				<td>${supermarket_in - supermarket_out}</td>
				<td>${line_in}</td>
				<td>${line_out}</td>
				<td>${line_in - line_out}</td>
				<td>${DATASET[x]['rej_qty']}</td>
				<td>${DATASET[x]['fg_qty']}</td>
				<td>${DATASET[x]['transfered_qty']}</td>
				<td>${ship_qty * -1}</td>
				<td>${DATASET[x]['fg_wip_qty']}</td>
				</tr>`;
			}
      ...
      { "data": "color_code" },
          ...
          return str;
			...
			},
			{
				data: "fg_wip_qty",
				/*render : function(data, type, full, meta ){
					let fg_qty = full['fg_qty'] == null ? 0 : parseInt(full['fg_qty']);
					let ship_qty = full['ship_qty'] == null ? 0 : parseInt(full['ship_qty']);
					let str = `<span style="color:red">${fg_qty - (ship_qty * -1)}</span>`;
          return str;
				}*/
			}
    ];
