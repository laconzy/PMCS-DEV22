		...
		appAjaxRequest({
                ...
                success: function (response) {
                    ...
                    $('#mdl-tbl-ratio').append(
						'<tr>' +
						'<td style="text-align: center">' + detail[i]['line_no'] + '</td>' +
						'<td style="text-align: center">' + detail[i]['size_code'] + '</td>' +
						'<td style="text-align: center">' + detail[i]['ratio'] + '</td>' +
						'<td style="text-align: right">' + detail[i]['plies'] + '</td>' +
						'<td style="text-align: right">' + (detail[i]['ratio'] * detail[i]['plies']) + '</td>' +
						'<td style="text-align: left">' + detail[i]['marker_ref'] + '</td>' +
						'<td style="text-align: center">' + detail[i]['width'] + '</td>' +
						'<td style="text-align: center">' + detail[i]['width'] + '</td>' +
						'<td>' + rem_button + '</td>' +
						'</tr>');
                    }
    ...
    $("#mdl-btn-ratio").click(function (event) {
        ...
        if (obj !== undefined && obj !== false)
        {
			...
			if(plies <= 0 )
                ...
                if(status == '0' )
                { 
                    appAlertError('Please Select the Status');
                    return true; 
                }
