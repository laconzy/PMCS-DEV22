...
$(document).ready(function() {
    ...
    TABLE = $('#embellishment-table').DataTable( {
      "scrollY": "500px",
		  "scrollX": true,
		  "scrollCollapse": true,
      "autowidth":false,
		  ...
		  "serverSide": true,
        "ajax": {
            "url": BASE_URL+"index.php/master/embellishment/get_embellishments",
            "type": "POST",
            "data" : {
...
$('#embellishment-table').on('click','button',function(){
	var ele = $(this);
	var type = ele.attr('data-btn');
	if( type === 'view')
		open_embellishment(this);
	else if(type === 'delete' )
		delete_embellishment(this);
});

function open_embellishment(ele)
{
    //alert($(ele).attr('data-id'));
    window.open(BASE_URL+'index.php/master/embellishment/embellishment_view/'+$(ele).attr('data-id'));
}


function delete_embellishment(ele)
{
  var emb_id = $(ele).attr('data-id');
  appAlertConfirm('Do you want to deactivate selected embellishment type?' , function(){
    appAjaxRequest({
      url : BASE_URL + 'index.php/master/embellishment/destroy/'+emb_id,
      type: 'post',
      dataType : 'json',
