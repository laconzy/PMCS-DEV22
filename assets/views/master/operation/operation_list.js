...
$(document).ready(function() {
    ...
    TABLE = $('#sites-table').DataTable( {
      ...
      "autowidth":false,
		  ...
		  "serverSide": true,
        "ajax": {
            "url": BASE_URL+"index.php/master/operation/get_operations",
            "type": "POST",
            "data" : {
...
$('#sites-table').on('click','button',function(){
	...
	if( type === 'view')
		...
		delete_operation(this);
});

function open_operation(ele)
{
    //alert($(ele).attr('data-id'));
    window.open(BASE_URL+'index.php/master/operation/operation_view/'+$(ele).attr('data-id'));
}


function delete_operation(ele)
{
  var color_id = $(ele).attr('data-id');
  appAlertConfirm('Do you want to deactivate selected operation?' , function(){
    appAjaxRequest({
      url : BASE_URL + 'index.php/master/operation/destroy/'+color_id,
      type: 'post',
      dataType : 'json',
