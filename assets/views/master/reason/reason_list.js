...
$(document).ready(function() {
    ...
    TABLE = $('#reason-table').DataTable( {
      ...
      "autowidth":false,
		  ...
		  "serverSide": true,
        "ajax": {
            "url": BASE_URL+"index.php/master/reason/get_reasons",
            "type": "POST",
            "data" : {
...
$('#reason-table').on('click','button',function(){
	var ele = $(this);
...
function open_reason(ele)
{
    //alert($(ele).attr('data-id'));
    window.open(BASE_URL+'index.php/master/reason/reason_view/'+$(ele).attr('data-id'));
}


function delete_reason(ele)
{
  var reason_id = $(ele).attr('data-id');
  appAlertConfirm('Do you want to deactivate selected reason?' , function(){
    appAjaxRequest({
      url : BASE_URL + 'index.php/master/reason/destroy/'+reason_id,
      type: 'post',
      dataType : 'json',
