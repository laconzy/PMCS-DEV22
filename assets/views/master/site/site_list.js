...
$(document).ready(function() {
    ...
    TABLE = $('#sites-table').DataTable( {
      ...
      "autowidth":false,
		  ...
		  "serverSide": true,
        "ajax": {
            "url": BASE_URL+"index.php/master/site/get_sites",
            "type": "POST",
            "data" : {
...
function open_site(ele)
{
    ...
    window.open(BASE_URL+'index.php/master/site/site_view/'+$(ele).attr('data-id'));
}


function delete_site(ele)
{
  var site_id = $(ele).attr('data-id');
  appAlertConfirm('Do you want to deactivate selected site?' , function(){
    appAjaxRequest({
      url : BASE_URL + 'index.php/master/site/destroy/'+site_id,
      type: 'post',
      dataType : 'json',
