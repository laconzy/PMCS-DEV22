...
$(document).ready(function() {
    ...
    TABLE = $('#supplier-table').DataTable( {
      ...
      "autowidth":false,
		  ...
		  "serverSide": true,
        "ajax": {
            "url": BASE_URL+"index.php/master/supplier/get_list",
            "type": "POST",
            "data" : {
...
$('#supplier-table').on('click','button',function(){
	var ele = $(this);
	var type = ele.attr('data-btn');
	if( type === 'view')
		open_supplier(this);
	else if(type === 'delete' )
		delete_supplier(this);
});

function open_supplier(ele)
{
    //alert($(ele).attr('data-id'));
    window.open(BASE_URL+'index.php/master/supplier/show/'+$(ele).attr('data-id'));
}


function delete_supplier(ele)
{
  var sup_id = $(ele).attr('data-id');
  appAlertConfirm('Do you want to deactivate selected supplier?' , function(){
    appAjaxRequest({
      url : BASE_URL + 'index.php/master/supplier/destroy/'+sup_id,
      type: 'post',
      dataType : 'json',
